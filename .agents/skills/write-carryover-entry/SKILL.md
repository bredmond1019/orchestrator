...
description: >-
  ...
  carryover[] at all rather than an operator edge, a block, or reference[], how to author a
  clears_when predicate that will still fire in six weeks, how to write text that survives its
  author being wrong, and the finding_id discipline that makes one finding one fix. Use BEFORE
  adding any carryover[] entry, at every /handoff, /wrap-up, /log-work and lane close, before
  filing a finding an orchestration run turned up, and when a sweep reports an entry CLEARED
  that is plainly still live.
---
