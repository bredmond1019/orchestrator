...
description: >
  How to reach the human operator mid-run over `bastion notify send|ask` without recreating the
  noise that got the old Stop/Notification hook retired — when NOT to notify (restraint first),
  which verb to use, the durable-home rule inherited from ping-agent, the outcome table with an
  action per status, payload composition limits, and the multi-bot contract. Use BEFORE calling
  `bastion notify` for any reason, and BEFORE deciding a lane is blocked and the operator should be
  told.
---
