"""Tests for the assembled DocumentQAWorkflow (Project D, Task 4).

Two layers:

* **Structure** — registration, schema wiring (start node, linear five-node
  connections) and that ``WorkflowValidator`` accepts the graph.
...
class TestDocumentQAWorkflowStructure:
    ...
    def test_five_nodes_in_workflow(self):
        """The workflow must contain exactly five nodes."""
    ...
    def test_linear_dag_connections(self):
        """Connections form a strict linear chain: Embed->Retrieve->Assemble->Answer->Update->[]."""
        node_map = {
        ...
        assert node_map[RetrieveChunksNode] == [AssembleContextNode]
