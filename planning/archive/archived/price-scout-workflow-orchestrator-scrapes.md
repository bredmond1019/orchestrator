---
type: Plan
title: Price Scout Workflow — deferred plan (ARCHIVED — orchestrator-scrapes framing)
description: ARCHIVED 2026-06-22. Original deferred plan that had the orchestrator do the scraping (Playwright SearchNode → ExtractNode → RankNode inside a Celery worker). Superseded by the D3 pivot — Price Scout now scrapes/ranks standalone and emits a finished result via /events. See planning/plans/price-scout-workflow.md.
---

> **ARCHIVED 2026-06-22.** This is the original version of `price-scout-workflow.md`, preserved
> for reference. It is **obsolete**: it assumes the orchestrator owns the browser and does the
> scraping (Playwright nodes in a Celery worker). Under D3 (`price-scout` repo), Price Scout is a
> standalone app that scrapes, enriches, and ranks on its own and sends this orchestrator an
> already-finished result via `POST /events/`. The current plan (ingest + analyze) lives at
> `planning/plans/price-scout-workflow.md`.
