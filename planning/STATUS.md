...
**Last updated:** 2026-06-08 — Block C in progress (Tasks 1–8 complete; Tasks 9–14 next — comprehensive unit tests + validation)
**Current focus:** Phase 0, Block C — Task 9: write `BaseRouter` and `RouterNode` unit tests
...
| C | Test infra + core hardening + 4 bug fixes | In progress | Tasks 1–8 complete (pytest scaffold; `GenericRepository.exists()` fix; import-time side effects in `session.py`/`worker/config.py` fixed; ghost-row bug in `api/endpoint.py` fixed; router key coupling fix — `TaskContext.get_node_output()` added; `TaskContext` + `WorkflowSchema` unit tests written; `WorkflowValidator` unit tests written; `Workflow.run()` unit tests written); Tasks 9–14 next (BaseRouter/RouterNode, ParallelNode, PromptManager, GenericRepository CRUD tests + validation) |
