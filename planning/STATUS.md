...
**Last updated:** 2026-06-10 — Block B public face live (`learn-agentic-ai.com` via Cloudflare Tunnel); Block B (Tailscale) + Block D now run in parallel
**Current focus:** Phase 0, Block B — private face / Tailscale **and** Block D — Shared services + first scaffold (parallel)
...
| B | Mac Mini agentic harness + revive site | In progress | Public face DONE: `learn-agentic-ai.com` live to the public via Cloudflare Tunnel + Cloudflare DNS on the Mac Mini. Remaining: private face — install Tailscale on Mini + all devices (Pixel tablet, phone, Kindle, laptop) and put private tooling (personal feed, orchestration API, Celery) behind it, no open ports. See DECISIONS D23 |
