...
| Lever | How |
|---|---|
| **Model tiering** | Principle: **match the model to the work** — Opus plans, Haiku does the mechanics, Sonnet does the judgment between. In the orchestrator, only the dependency-graph **analysis** (planning) runs on **opus**; **sonnet** drives triage, merge, and report; **haiku** handles teardown and plan-writing. The child `/sdlc-task` (and its sequential sibling `/sdlc-run`, now tiered identically) follows the same rule — **opus** only for the spec-authoring fallback (`generate-tasks`); **haiku** for the purely-mechanical stages (`scout`, `test`, `finalize` — fixed procedures with no judgment, and `test` is backstopped by review's authoritative fresh-test run); **sonnet** for the judgment work (implement/fix/review/document/log). See each file's `MODEL` map. The real planning leverage is upstream: run the `/generate-tasks` and `/breakdown` skills on an Opus session. |
