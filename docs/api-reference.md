...
| `--prune-failed` | Opt-in: delete a parse-failed file's pre-existing `brain_documents` rows instead of quarantining (retaining) them. Dry-run-aware. |

**OR.2.C — corpus integrity at ingest.** A malformed file (invalid YAML frontmatter) raises a typed
`DocumentParseError` (`file_path` + cause), caught per-file so the run logs and skips it rather than
aborting; `main()` returns a real exit code (`1` on any parse/embed/DB failure, propagated via
`sys.exit(main())`), not an implicit `None`. A parse-failed file's pre-existing rows are retained
(quarantined) by default and reported as `stale_rows_retained` in the run summary; `--prune-failed`
deletes them instead. See `docs/scripts.md` § `index_brain.py` for the full walkthrough.
...
### `app/brain/ops.py` — the Brain write/ops core

```python
class UnknownRoutineError(Exception): ...   # name not in ROUTINES
class MevUnavailableError(Exception): ...   # `mev` binary not on PATH
...
DEFAULT_QUERY_KEEP_DAYS: int = 90        # `retrieval_queries` retention window

ROUTINES: dict[str, Callable[[], dict]]  # {"refresh", "stale", "reconcile", "eval", "queries_prune"}
...
| Function | Description |
|---|---|
| `embed_paths(paths, *, force=False, brain_path=None)` | Re-embeds exactly the named files by shelling into `index_brain.main(["--only-paths", *paths, ...])` via the shared `_run_index_brain` choke point — no second chunk->embed->write implementation. `force=True` forwards `--force` (bypass the incremental skip). Returns `{"embedded": [...], "forced": bool, "exit_code": int, "success": bool, "errors": [...]}` (`OR.2.C` task 3) — `exit_code`/`errors` are `index_brain.main()`'s real return value and captured ERROR-level log messages for this call, and `success` is `exit_code == 0`, so `syn embed` exits non-zero on a reported parse/embed/DB failure instead of always reporting a clean run. |
| `ingest_dir(directory, *, force=False, brain_path=None)` | Collects every `*.md` file under `directory` (`Path.rglob`, sorted) and calls `embed_paths` on the full list. Raises `NotADirectoryError` when `directory` doesn't exist or isn't a directory. A directory with no matching files returns `{"ingested": [], "forced": force, "exit_code": 0, "success": True, "errors": []}` without calling `embed_paths` — otherwise forwards `embed_paths`'s `exit_code`/`success`/`errors` verbatim (no direct `index_brain.main()` call site of its own). This is on-disk *file* indexing (frontmatter parsing, `doc_type` classification, `authored_at` from mtime) — a different concern from `app/brain/ingest.py::ingest_artifact`'s arbitrary-content API path; do not route through the OR.Q ingest core. |
| `prune_paths(paths, *, dry_run=False, brain_path=None)` | Deletes `brain_documents` rows for deleted/renamed-away file paths by shelling into `index_brain.main(["--prune-paths", *paths, ...])` via `_run_index_brain` — no embedding, no API call. The single implementation shared by `syn prune` and the brain repo's post-commit delete/rename freshness hook (which previously called `scripts/index_brain.py --prune-paths` directly). Returns `{"pruned": [...], "dry_run": bool, "exit_code": int, "success": bool, "errors": [...]}` — `--prune-paths` exits before the corpus walk that can raise `DocumentParseError`, so these are `0`/`True`/`[]` today, captured anyway so the contract matches `embed_paths`/`refresh`. |
| `prune_queries(keep_days=None, *, dry_run=False)` | Retention for the OR.K1 `retrieval_queries` log — the bounded, idempotent delete that closes its unbounded-growth gap. Deletes rows whose `created_at` is **strictly older than** `now - keep_days` (a row exactly at the cutoff is KEPT). Window resolution is **explicit argument > `BRAIN_QUERY_LOG_KEEP_DAYS` env var > `DEFAULT_QUERY_KEEP_DAYS` (90)**, with the env var read at *call* time, mirroring `query_log._query_log_enabled`'s discipline; an unparsable or non-positive value falls back to 90 with a `logging.warning` rather than crashing an unattended cron run. The delete mirrors `_delete_orphaned_chunks`'s bounded shape (one filtered delete, one commit) rather than `GenericRepository.delete`'s row-at-a-time commits; `deleted`/`kept` are derived from real `GenericRepository.count()` reads around the delete, never from what the caller requested. `dry_run=True` counts and deletes nothing (`deleted` then reports the would-delete count, disambiguated by the `dry_run` flag — the same convention `prune_paths` uses). Returns `{"deleted": n, "kept": m, "cutoff": iso, "keep_days": d, "dry_run": bool}`. **Deletion, never aggregation** (the D51 guard): nothing is rolled up or persisted at prune time, and `syn queries` stays the only read surface, still computing over raw rows. |
...
| `refresh(*, rebuild=False, dry_run=False, brain_path=None)` | Runs the content-index step (`index_brain.main` via `_run_index_brain`) then the edge-reload step (`refresh_edges`), in that order. `dry_run=True` skips the edge step entirely (`brain_edges` has no dry-run equivalent) and returns `{"documents": {"dry_run": True, "exit_code": int, "success": bool, "errors": [...]}, "edges": {"skipped": True}}`; otherwise returns the same `documents` shape (`dry_run: False`) with `{"edges": {"loaded": N}}`. `documents.success` is `False` whenever `index_brain.main()` reported a parse/embed/DB failure, so `syn refresh`/`syn routine refresh` surface it instead of always reporting a clean run. |
...
| `run_routine(name)` | Dispatches to `ROUTINES[name]()`. Raises `UnknownRoutineError` (listing the known names) for an unregistered name. `ROUTINES` currently registers `"refresh"`, `"stale"`, `"reconcile"` (the deep check, report-only — `_reconcile_routine` calls `reconcile.deep_stale().to_dict()` with no `--repair` dispatch, since a cron routine must be safe to run unattended), `"eval"`, and `"queries_prune"` (`prune_queries()` with defaults — the one **destructive** entry, and deliberately so: `reconcile`/`eval` are report-only because they would otherwise *repair* or *write*, which is open-ended; a retention prune is instead a bounded, idempotent delete of rows past a fixed window, which is exactly what a cron routine is for); entries are lambdas (not direct function references) specifically so `patch("app.brain.ops.refresh", ...)` / `patch("app.brain.ops.stale", ...)` affects dispatch — a direct reference would bind the original function object at import time. This is the convention `OR.J`'s cron invokes. |
...
module is explicitly not fixing there). Five drift axes plus two informational lanes, each backed by
...
| `section_orphans` | `_section_orphans` | A `(file_path, section)` pair in the DB absent from the file's *current* section set (`brain.chunking.chunk_by_section` over `index_brain.parse_document`'s body) for a still-existing file — a header rename/removal leaks a row the `(file_path, section)`-keyed upsert never revisits. A file already reported under `deleted_but_embedded` is skipped here (no double-report). A malformed file (`DocumentParseError`) is caught and added to axis 7's `unparseable_files` instead of propagating — the `OR.2.C` fix for a bare `parse_document` call that used to make `deep_stale` itself raise on one broken file. |
...
| `uncovered_files` / `unparseable_files` / `excluded_count` | `_ingest_ceiling` (`OR.2.C`) | Axis 7, informational only: the inverse of axes 1-2 (filesystem -> DB instead of DB -> filesystem). `uncovered_files` — enumerated on disk via `index_brain._collect_files` (imported, never reimplemented) with zero `brain_documents` rows, the recall ceiling `syn eval` can never see past. `unparseable_files` — files whose YAML frontmatter fails to parse, whether flagged during `_collect_files`' own enumeration (the sub-repo `docs/` lane) or re-checked here. `excluded_count` — files the enumeration itself excluded, broken down by reason (`skip_dir`, `leading_underscore`, `ephemeral_filename`) via `_excluded_counts`, which imports `index_brain`'s exclusion predicates rather than reimplementing them. Degrades to `([], [], {})` when the resolved root carries no `brain.toml` (a throwaway test root). |
...
`True` when any of axes 1-5 is non-empty; `unstamped_count`, the `ingested/` lane, and axis 7's
three buckets are informational and never contribute to `drift`.
...
query — `graph.walk()` in this block takes no `workspace` parameter. The five write/ops commands
share a uniform error path (`_emit_error`): any exception raised by the underlying `brain.ops`
