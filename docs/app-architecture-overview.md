...
| Project C — Task 3 | `workflows/proposal_generator_workflow_nodes/opportunity_identifier_node.py`, `prompts/proposal_opportunity_identifier.j2` | `OpportunityIdentifierNode(AgentNode)` reads `company_name`, `industry`, `description`, and `intake_notes` from the event plus the structured research brief from `CompanyResearchNode` output in `TaskContext`; serializes all context into a JSON user prompt; runs the agent; validates the `ScoredCandidate` composite formula via `model_validator` at parse time (wrong composite raises `ValidationError` before `process()` is reached); writes `candidates` (list of dicts, composite-desc sorted) and `recommended` (str) to `TaskContext` under the node's name. `ModelProvider.CLAUDE_CODE_SDK` / `"sonnet"`. `proposal_opportunity_identifier.j2` embeds all five rubric axis anchor levels for `frequency`, `time_cost`, and `buildability` (1–5 each) and the binding composite formula — scoring is model-version-stable without Python changes. |
...
| Project C — Task 6 | `workflows/proposal_generator_workflow_nodes/storage_node.py` | `StorageNode(Node)`: terminal persistence node for the proposal generator. Reads the final `AutomationRoadmap` from `ReviseNode` output (revise branch) or `ProposalWriterNode` output (pass branch). Builds an embed string from `roadmap.situation_summary` + candidate names, calls `EmbeddingService().embed_text(embed_text)`, and persists a `BrainDocument(doc_type="proposal", section="AutomationRoadmap", file_path=f"proposals/{artifact_id}/roadmap.json")` via `GenericRepository` + `db_session` factory (rule 7 — no deployment logic). Captures `artifact_id` from `task_context.event.artifact_id` before the session commits to prevent `DetachedInstanceError`. Output keys: `artifact_id`, `file_path`, `company_name`, `embedded`, `doc_type`. |
