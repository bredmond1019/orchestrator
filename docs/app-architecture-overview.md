...
- `BrainDocument` — **built** (`app/database/brain_document.py`, brain-rag Layer 1 + frontmatter-indexer-enrich); `Vector(1024)` embedding + OKF metadata columns (`doc_id`, `layer`, `project`, `status`, `keywords`, `related`) + `ARRAY(String)` workflow_patterns; populated by `scripts/index_brain.py`. The base table is migration `b3c4d5e6f7a8` (chains off `a1b2c3d4e5f6`); the six OKF columns + their GIN/btree indexes were added later by migration `d1e2f3a4b5c6` (chains off `c4d5e6f7a8b9`, the content-chunks migration). Query path available via `RetrieveChunksNode` corpus `"brain"` with optional metadata filters (built in Project D Task 3 + frontmatter-retrieval-filters).
- `ContentChunk` — **built** (`app/database/content_chunk.py`, Phase 1 Project D Task 1); `Vector(1024)` embedding column + `is_section_title` flag (written at ingest, surfaced on retrieval results; ranking-neutral since `OR.ticket.section-title-boost`); `doc_id` indexed. Migration `c4d5e6f7a8b9` chains off `b3c4d5e6f7a8`.
...
`customer_care`; all four are now removed under `OR.X` (D51), and `SDLC_FLOW` was removed under
`OR.X2`. The registry that survives holds `DOCUMENT_INGEST`, `DOCUMENT_QA`, `MEMORY_INGEST`, and
