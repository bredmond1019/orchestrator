...
| `POST` | `/ingest/artifact` | `{ "artifact_id": str, "doc_type": str \| null, "content": str \| null, "section": str \| null, "project": str \| null, "title": str \| null, "description": str \| null, "metadata": object \| null, "authored_at": datetime \| null, "channel_type": str \| null, "source_ref": str \| null, "summary": str \| null, "digest_markdown": str \| null, "entities": [str, ...] \| null, "language": str \| null }` | `200 { "artifact_id": str, "chunks_written": int }` — **ingest** a generic artifact (new in v1.3.0; `authored_at` added v1.5.0; six optional `LearningArtifact` fields — `channel_type`, `source_ref`, `summary`, `digest_markdown`, `entities`, `language` — plus nullable-with-fallback `doc_type`/`content` added v1.8.0) |
| `GET` | `/recall?q=<str>&limit=<int>&hybrid=<bool>` | — | `200 { "query": str, "count": int, "results": [ { "doc_id": str \| null, "file_path": str, "title": str \| null, "section": str \| null, "content": str, "score": float, "via": str }, ... ] }` — **recall** corpus results (new in v1.4.0; `score` polarity and `via` vocabulary changed in v1.6.0 — see changelog) |
...
- **`POST /ingest/artifact`** accepts the generic envelope for other producers (`EN.5.A`
  ...
  Since v1.8.0 it is also the accepted route for engine-rs's content-pipeline `LearningArtifact`
  shape — its literal seven fields (`artifact_id`, `channel_type`, `source_ref`, `summary`,
  `digest_markdown`, `entities`, `language`) — as a first-class body, not a hand-mapped
  approximation: `doc_type` and `content` are now nullable and fall back at the route
  (`app/api/ingest.py`) to `"learning_artifact"` and `digest_markdown` respectively, and
  `channel_type`/`source_ref`/`summary`/`entities`/`language` fold into the outbound `metadata`
  alongside any caller-supplied `metadata` (caller-supplied keys win). At least one of
  `content`/`digest_markdown` and one of `doc_type`/`digest_markdown` must be present or the
  request 422s. **`/ingest/learning` has never existed** — this route absorbs the
  `LearningArtifact` shape instead of a new route being added for it.
